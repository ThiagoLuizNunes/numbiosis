function cholesky() {
  ...
  addStep(
    ...
    (matrix = maTex(origin, "A =") + maTex(matrixG, "G =") + maTex(matrixGT, "G^T ="))
  );
  ...
  addStep(title = "Equação para encontrar o Fator de Cholesky",
  ...
  for (let j = 0; j < origin.length; j++) {
    ...
    for (let i = j; i < origin.length; i++) {
      ...
      if (i == j) {
        ...
        matrixGWValues[i][j] = Math.round(Math.sqrt(origin[i][j] - factorFinal) * 1000) / 1000;
        ...
        equation = underIndex('g', (i + 1) + '' + (j + 1))
          + ' = \\sqrt{'
          + underIndex('a', (i + 1) + '' + (j + 1))
          + factorEquation
          + "}";

        equationWValues = underIndex('g', (i + 1) + '' + (j + 1))
          + ' = \\sqrt{'
          + origin[i][j]
          + factorEquationWValues
          + "}";

        equationResult = underIndex('g', (i + 1) + '' + (j + 1)) + ' = ' + matrixGWValues[i][j];
        ...
        for (let n = 0; n < j; n++) {
          ...
          factorEquation += '-' + underIndex('g', (i + 1) + '' + (n + 1)) + ' * ' + underIndex('g', (j + 1) + '' + (n + 1));
          factorEquationWValues += '-' + negative(matrixGWValues[i][n]) + "*" + negative(matrixGWValues[j][n]);

        }

        matrixGWValues[i][j] = Math.round((origin[j][i] + factorFinal) / matrixGWValues[j][j] * 1000) / 1000;

        equation = underIndex('g', (i + 1) + '' + (j + 1))
          + ' = \\frac{' + underIndex('a', (i + 1) + '' + (j + 1))
          + factorEquation
          + "}{"
    ...
    addStep(title = "Coluna " + (j + 1),
      matrix = maTex(matrixGWValues, "G ="),
      equations = finalEquations);
  }

  addStep(title = "Fator de Cholesky", matrix = maTex(matrixGWValues, 'G ='));
}
